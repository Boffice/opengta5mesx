...
function notifyAlertSMS (source, number, alert, listSrc)
  local _source = source
  if PhoneNumbers[number] ~= nil then
	local mess = "error"
	if alert.numero then
		mess = 'From #' .. alert.numero  .. ' : ' .. alert.message
	else
		mess = 'From Unknown : ' .. alert.message
	end 
    ...
    if count == 0 then
      TriggerClientEvent('esx:showNotification', _source, "Nobody available to receive message")
    end
  else
      TriggerClientEvent('esx:showNotification', _source, "Nobody available to receive message")
  end
...
function getPhoneNumber (source, callback) 
  ...
  print(("GCHONE-addones: %s"):format(xPlayer.identifier))
  print(("GCHONE-addones: %s"):format(xPlayer.phone_number))
  MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier',{
    ['@identifier'] = xPlayer.identifier
  }, function(result)
