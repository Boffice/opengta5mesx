...
local seatbeltEjectAccel = 150.0 * Config.vehicle.ejectpower
		...
		if IsPedInAnyVehicle(player, false) and vehicleIsOn then
			...
			if has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~= 8 then
                ...
                if not seatbeltIsOn then
					...
					--print(vehIsMovingFwd)
					--print(prevSpeed)
					--print(seatbeltEjectSpeed/2.237)
					--print(vehAcc)
					--print(seatbeltEjectAccel*9.81)
                    if (vehIsMovingFwd and (prevSpeed > seatbeltEjectSpeed) and (vehAcc > seatbeltEjectAccel)) then
					--if prevSpeed ~= nil and GetEntitySpeedVector(vehicle, true).y > 1.0 and currSpeed > 19.25  and (prevSpeed - currSpeed) > (currSpeed * 0.255) then
	...
	if Config.ui.showWeapons == true then
		while true do
			...
			if IsPedArmed(player, 7) then
				...
				--print(("[trew_hud_ui] [^2INFO^7] weapon: %s"):format(weapon))
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlPressed(0, Keys[',']) then
		    disableui = not disableui
		    SendNUIMessage({ action = 'toggleUi', value = disableui })
		end
	end
end)
