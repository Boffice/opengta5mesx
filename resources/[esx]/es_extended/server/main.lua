...
function loadESXPlayer(identifier, playerId)
	...
	Async.parallel(tasks, function(results)
		...
		print(('[es_extended] [^2INFO^7] Creating playey: %s - #accounts: %s'):format(userData.playerName, #(userData.accounts)))
		xPlayer.getMissingAccounts(function(missingAccounts)
			if #missingAccounts > 0 then
				for k,v in ipairs(missingAccounts) do
					print(('[es_extended] [^2INFO^7] Creating account: %s - value: %s'):format(v, 'na'))

					local money = Config.DefaultAccounts[v]
					table.insert(xPlayer.accounts, {
						name = v,
						money = money,
						label = Config.Accounts[v]
					})
					print(('[es_extended] [^2INFO^7] Creating account: %s - value: %s'):format(v, money))
