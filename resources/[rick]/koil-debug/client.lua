...
function GetDoor()
    local playerped = GetPlayerPed(-1)
    local playerCoords = GetEntityCoords(playerped)
    -- local obs, distance = ESX.Game.GetClosestObject(doortype, playerCoords)
    local objects = ESX.Game.GetObjects()
    for i=1, #objects, 1 do
		print(string.format("DEBUG: %s - %s - %s", i, objects[i], GetEntityModel(objects[i])))
    end
end
...
function GetClosestObject()
    ...
    print(string.format("Coil DEBUG:num of obj near - %s", #objects))
    for i=1, #objects, 1 do
        local objectCoords = GetEntityCoords(objects[i])

        local distance = #(objectCoords - coords)

        --print(string.format("Coil DEBUG: %s - %s", closestObject, closestDistance))
        --print(string.format("Coil DEBUG: x: %s - y: %s - z :%s", objectCoords["x"],objectCoords["y"],objectCoords["z"]))
        --print(string.format("Coil DEBUG: %s - %s", objects[i], GetEntityModel(objects[i])))
        if distance < 10 then
            DrawText3Ds(objectCoords["x"],objectCoords["y"],objectCoords["z"], "Obj: " .. objects[i] .. " Model: " .. GetEntityModel(objects[i]) .. "" )
        end
    ...
    --print(string.format("Coil DEBUG: %s - %s", closestObject, closestDistance))
    --print(string.format("Coil DEBUG: x: %s - y: %s - z :%s", objectCoords["x"],objectCoords["y"],objectCoords["z"]))
    --print(string.format("Coil DEBUG: %s - %s", closestObject, GetEntityModel(closestObject)))
...
Citizen.CreateThread( function()
    ...
    while true do
        ...
        if dickheaddebug then
            ...
            local c_obj, c_dist = GetClosestObject()
            local objectCoords = GetEntityCoords(c_obj)
            --print(string.format("Coil DEBUG: %s - %s", c_obj, c_dist))
            --print(string.format("Coil DEBUG: x: %s - y: %s - z :%s", objectCoords["x"],objectCoords["y"],objectCoords["z"]))
            --print(string.format("Coil DEBUG: %s - %s", c_obj, GetEntityModel(c_obj)))
            --print(string.format("Coil DEBUG: Exists - %s", DoesEntityExist(c_obj)))


            -- Draw it
            -- drawTxt(0.8, 0.66, 0.4,0.4,0.30, "TEST", 55, 55, 155, 255)
            drawTxt(0.8, 0.70, 0.4, 0.4, 0.30, "Closest Object", 55, 55, 155, 255)
            drawTxt(0.8, 0.72, 0.4, 0.4, 0.30, "Coords: " .. objectCoords, 55, 55, 155, 255)
            drawTxt(0.8, 0.74, 0.4, 0.4, 0.30, "Model : " .. GetEntityModel(c_obj), 55, 55, 155, 255)
            drawTxt(0.8, 0.76, 0.4, 0.4, 0.30, "Distance: " .. c_dist, 55, 55, 155, 255)
            drawTxt(0.8, 0.78, 0.4, 0.4, 0.30, "Object: " .. c_obj, 55, 55, 155, 255)
            drawTxt(0.8, 0.80, 0.4, 0.4, 0.30, "Exists: " .. DoesEntityExist(c_obj), 55, 55, 155, 255)
