...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_doorlock:getDoorState', function(doorState)
		for index,state in pairs(doorState) do
			...
			print(string.format("ESX-DOORLOCK: Door: %s",Config.DoorList[index].objHash ))
		end
	end)
end)
		...
		for k,v in ipairs(Config.DoorList) do
			...
			if v.doors then
				for k2,v2 in ipairs(v.doors) do
					--print(string.format("ESX-DOORLOCK: door pair exists"))
				...
				if v.object and DoesEntityExist(v.object) then
					--print(string.format("ESX-DOORLOCK: DoorHash: %s EXISTS", v.objHash ))
					--print(string.format("ESX-DOORLOCK: DoorObject: %s EXISTS", type(v.object )))
					--print(string.format("ESX-DOORLOCK: DoorObject: %s EXISTS", v.object ))
					v.distanceToPlayer = #(playerCoords - GetEntityCoords(v.object))
					--print(string.format("ESX-DOORLOCK: distance: %s", v.distanceToPlayer ))

					if v.locked and v.objHeading and ESX.Math.Round(GetEntityHeading(v.object)) ~= v.objHeading then
						SetEntityHea0002dsfsdfding(v.object, v.objHeading)
					end
				else
					v.distanceToPlayer = nil
					--print(string.format("ESX-DOORLOCK: DoorHash: %s", v.objHash ))
					v.object = GetClosestObjectOfType(v.objCoords, 1.0, v.objHash, false, false, false)
					if not v.object then
						v.object = GetClosestObjectOfType(v.objCoords, 2.0, v.objHash, false, false, false)
						--print(string.format("ESX-DOORLOCK: Found on 2nd try"))
						if not v.object then
							--print(string.format("ESX-DOORLOCK: DoorHash: %s EXISTS", v.objHash ))
						end
					end
					if v.object then
						--print(string.format("ESX-DOORLOCK: DoorObject: %s", type(v.object )))
						--print(string.format("ESX-DOORLOCK: DoorObject: %s", v.object ))
						--print(string.format("ESX-DOORLOCK: DoorHash: %s EXISTS: %s", v.objHash, DoesEntityExist(v.object) ))
						local objectCoords = GetEntityCoords(v.object)
            		--print(string.format("Coil DEBUG: %s - %s", c_obj, c_dist))
						--print(string.format("ESX-DOORLOCK: x: %s - y: %s - z :%s", objectCoords["x"],objectCoords["y"],objectCoords["z"]))
						--print(string.format("ESX-DOORLOCK: %s - %s", v.object, GetEntityModel(v.object)))
