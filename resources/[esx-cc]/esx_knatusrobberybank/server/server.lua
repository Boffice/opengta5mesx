...
AddEventHandler('esx_holdupbank:rob', function(robb)
	...
	if Banks[robb] then
		...
		local timebetween = Config.TimeBetween
		if bank.robtimeout then
			timebetween = bank.robtimeout
		end
		if (os.time() - bank.lastrobbed) < timebetween and bank.lastrobbed ~= 0 then
			TriggerClientEvent('esx:showNotification', source, _U('already_robbed') .. (timebetween - (os.time() - bank.lastrobbed)) .. _U('seconds'))
			return
		end
		...
		if rob == false then
			...
			if xPlayer.getInventoryItem('blowtorch').count >= 1 then
				...
				local cops_required = Config.NumberOfCopsRequired
				if bank.num_cops_req then
					cops_required = bank.num_cops_req
				end
				if(cops >= cops_required)then
					...
					local rob_length = Config.RobLength
					if Banks[robb].rob_length then
						rob_length = Banks[robb].rob_length
					end
					print(rob_length)

					TriggerClientEvent('esx:showNotification', source, _U('started_to_rob') .. bank.nameofbank .. _U('do_not_move'))
					TriggerClientEvent('esx:showNotification', source, _U('alarm_triggered'))
					TriggerClientEvent('esx:showNotification', source, _U('hold_pos1') .. rob_length .. _U('hold_pos2'))
					...
					SetTimeout(rob_length * 1000, function()
...
AddEventHandler('esx_holdupbank:plantbomb', function(robb)
	...
	if Banks[robb] then
		...
		if(cops >= cops_required)then
			...
			TriggerClientEvent('esx:showNotification', source, _U('min_two_police') .. ": " .. cops_required)
		end
