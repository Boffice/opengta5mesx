            ...
            if IsInWeaponShopZone(coords) then
                if IsControlJustReleased(0, Keys["E"]) then
                    if Licenses['weapon'] ~= nil then
                        ...
                        exports['mythic_notify']:DoCustomHudText('error', 'You need a Fire Arms license before you can buy weapons')
                    end
                end
            end
        end
        ...
        if GetDistanceBetweenCoords(coords, Config.WeaponLiscence.x, Config.WeaponLiscence.y, Config.WeaponLiscence.z, true) < 3.0 then
            ...
            if IsControlJustReleased(0, Keys["E"]) then
                if Licenses['weapon'] == nil then
                    ...
                    --TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You already have a Fire arms license!' })
                    --exports['mythic_notify']:SendAlert('error', 'You already have a Fire arms license!')
                    exports['mythic_notify']:DoCustomHudText('error', 'You already have a Fire arms license!')
                end
                Citizen.Wait(2000)
            end
        end
